  ...
  pub fn resolve_burn(&mut self, token_id: AccountId, account_id: AccountId, amount: u128) {
    ...
    let token_decimals = match env::promise_result(1) {
      PromiseResult::Successful(val) => {
        match from_slice::<FungibleTokenMetadata>(&val) {
    ...
    let converted_session = amount.checked_div(u128::pow(10, token_decimals.into())).expect("Invalid division");
    let burn_amount = account.burn_amount.checked_add(amount).expect("Overflow token amount");
    let total_session = account.session.checked_add(converted_session).expect("Overflow account session");
